  ...
  const sortItems = useCallback(
    (items: PPartido[]): PPartido[] => {
      return [...items].sort((a, b) => {
        ...
        if (field === "name") {
          const nameA = a.nombrePartido?.toLowerCase() || "";
          const nameB = b.nombrePartido?.toLowerCase() || "";
          const result = nameA.localeCompare(nameB);
          return direction === "asc" ? result : -result;
        } else {
          const dateA =
            a.fechaFundacion && a.fechaFundacion !== "Unknown" ? parseISO(a.fechaFundacion) : new Date(0);
          const dateB =
            b.fechaFundacion && b.fechaFundacion !== "Unknown" ? parseISO(b.fechaFundacion) : new Date(0);
  ...
  const filterAndSortItems = useCallback(() => {
    ...
    if (selectedCategories.length > 0) {
      filtered = filtered.filter((item) =>
        selectedCategories.includes(item.nombrePartido),
      );
    }
